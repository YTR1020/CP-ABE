...
public class UserPrivateKey {
    private Element D;  //D=g^[(Alpha+r)/Beta]
    private PairingParameter pairingParameter;
    public Map<Attribute,Element> D_j_map;  //Dj=(g^r)*[H(j)^rj]
    public Map<Attribute,Element> D_j_pie_map;  //Dj'=g^rj
    private List<Attribute> userAttributes;  //用户属性集S

    private UserPrivateKey() {
        D_j_map = new HashMap<>();  //新建数据字典或者Hash查找的容器做 每个属性对应的Dj映射集
        D_j_pie_map = new HashMap<>();  //新建数据字典或者Hash查找的容器做 每个属性对应的Dj'映射集
    }
    ...
    public static UserPrivateKey build(MasterPrivateKey masterPrivateKey, List<Attribute> attributes){
        ...
        userPrivateKey.setPairingParameter(masterPrivateKey.getPairingParameter());  //根据系统主密钥，生成用户私钥
        userPrivateKey.setUserAttributes(attributes);  //获取用户属性集
        Element r = masterPrivateKey.getPairingParameter().getZr().newRandomElement().getImmutable();  //从Zr群中随机生成r
        Element alpha = masterPrivateKey.getAlpha();  //根据系统主密钥，获取Alpha
        Element beta = masterPrivateKey.getBeta();  //根据系统主密钥，获取Beta
        Element g = masterPrivateKey.getPairingParameter().getGenerator();  //根据系统主密钥，获取生成元g
        Element D = g.powZn((alpha.add(r)).div(beta)).getImmutable();  //计算D=g^[(Alpha+r)/Beta]
        userPrivateKey.setD(D);
        //遍历每个属性j∈S
        for (Attribute attribute : attributes){
            Element r_j = masterPrivateKey.getPairingParameter().getZr().newRandomElement().getImmutable();  //从Zr群中随机生成rj
            Element D_j = g.powZn(r).mul(masterPrivateKey.hash(attribute.getAttributeValue()).powZn(r_j)).getImmutable();
            //计算Dj=(g^r)*[H(j)^rj]
            Element D_j_pie = g.powZn(r_j).getImmutable();  //计算Dj'=g^rj
